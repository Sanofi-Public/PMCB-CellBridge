...
if (opt$genesets != "none") {
  ...
  p <- DotPlot(xsobj, group.by = "temp", features = gns,
               cluster.idents = TRUE, dot.min = 0.1) + 
    theme(axis.title = element_blank(),
          axis.text.y = element_text(size = ifelse(length(gns) < 30, 10, 8)),
          axis.text.x = element_text(size = 10, angle = 25, hjust = 1, vjust = 1),
          legend.position = "none") +
      coord_flip()
  if (length(gns) > 30) { p <- p + scale_x_discrete(guide = guide_axis(n.dodge=2))}
...
```{r eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE, fig.dim=c(4.0, 7.5), fig.align = 'center', results='asis'}
if (opt$genesets != "none") {
  make_tab <- function(x) {      # function to make the tabs
    cat("\n")                    # Space
    cat("####", names(x), "\n")  # Label tab
