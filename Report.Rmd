...
params:
    ...
    post.qc.summ.rna: NULL
    pre.qc.summ.adt: NULL
    post.qc.summ.adt: NULL
    trajs: NULL
    opt: NULL
    ui: NULL
---
...
```{r variable, eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE}
    ...
    fsobj <- params$fsobj
    sobj_noharm <- params$sobj_noharm
    meta_data <- params$meta_data
    pre.qc.summ.rna <- params$pre.qc.summ.rna
    post.qc.summ.rna <- params$post.qc.summ.rna
    pre.qc.summ.adt <- params$pre.qc.summ.adt
    post.qc.summ.adt <- params$post.qc.summ.adt
    trajs <- params$trajs
    opt <- params$opt
    ui <- params$ui
    n.sample <- length(obj_ls)
    samples <- names(obj_ls)
```
...
summ1 <- sum(pre.qc.summ.rna$cell)
summ2 <- sum(post.qc.summ.rna$cell)
# UMI per cell
...
```{r table-1, eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE, fig.align = 'center'}
  rm.mtdata <- c("project", "docker", "input", "help", "only_qc", "paper_url", "data_url")
  data.frame(opt) %>% 
...
for (i in 1:length(sobj_ls)) {
  sobj <- sobj_ls[[i]]
  # UMI per cell
  exprMat <- sobj@assays$RNA$counts
  countByCell <- colSums(exprMat)
...
rownames(umi_per_cell) <- names(sobj_ls)
rownames(gene_per_cell) <- names(sobj_ls)
rownames(mt_per_cell) <- names(sobj_ls)
```

## Sample-level Characteristics and Quality Control Metrics {.tabset}
...
pre.qc.summ.rna %>%
  ...
  dplyr::select(-type) %>%
  base::merge(., post.qc.summ.rna, by = c("sample_id")) %>%
...
`r eval_mrks <- ifelse(opt$adt, TRUE, FALSE)`

`r if (eval_mrks) { "### Cells & Genes (ADT)" }`

```{r, eval=eval_mrks, warning=FALSE, message=FALSE, echo=FALSE, fig.align = 'center'}
  pre.qc.summ.adt %>%
  dplyr::rename(pre_qc_cell = cell,
                pre_qc_gene = gene) %>%
  dplyr::mutate(pre_qc_cell = scales::comma(pre_qc_cell, accuracy = 1),
                pre_qc_gene = scales::comma(pre_qc_gene, accuracy = 1)) %>%
  dplyr::select(-type) %>%
  base::merge(., post.qc.summ.adt, by = c("sample_id")) %>%
  dplyr::rename(post_qc_cell = cell,
                post_qc_gene = gene) %>%
  dplyr::mutate(post_qc_cell = scales::comma(post_qc_cell, accuracy = 1),
                post_qc_gene = scales::comma(post_qc_gene, accuracy = 1)) %>%
  ...
  for (i in 1:length(obj_ls)) {
    ...
    db_plot <- db_plot %>%
      dplyr::bind_rows(sobj@meta.data %>%
                         ...
                         tidyr::gather(., key = type, value = feature, -barcode)%>%
...
`r if (keyval) {"## Doublet Cell Detection using Scrublet Scoring System {.tabset}"}`

`r if (keyval) {"Observed scores are used for doublet classification. Dashed line indicates the threshold used to identify doublets."}`
...
```{r eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE}
db_plot <- bind_rows(pre.qc.summ.rna, post.qc.summ.rna) %>%
...
db_plot <- merge(db_plot, post.qc.summ.rna, by = "sample_id", all = TRUE)
p2 <- ggplot(db_plot, 
...
## Key QC Metrics of Merged Samples
...
```{r eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE}
combo_ls <- list(c("fsobj","sample_id", "spring", "Signacx SPRING Projection"),
                 c("fsobj","sample_id", "umap", "Seurat UMAP Projection"),
                 c("fsobj","sample_id", "tsne", "Seurat TSNE Projection"),
                 c("sobj_noharm","sample_id", "umap", "Seurat UMAP Projection (no-batch correction)"),
                 c("sobj_noharm","sample_id", "tsne", "Seurat TSNE Projection (no-batch correction)"))
if (!opt$tsne) {
  idx <- which(unlist(lapply(combo_ls, function(x) any(grepl("tsne", x)))))
  combo_ls[idx] <- NULL
}
if (is.null(sobj_noharm)) {
  idx <- which(unlist(lapply(combo_ls, function(x) any(grepl("no-batch", x)))))
  combo_ls[idx] <- NULL
}
Q <- list()
for (i in 1:length(combo_ls)) {
  if (combo_ls[[i]][[1]] == "fsobj") {
    x <- fsobj
  } else {
    x <- sobj_noharm
  }
  p <- DimPlot(object = x, 
               reduction = combo_ls[[i]][3],
               group.by = combo_ls[[i]][2],
               label = TRUE, 
               repel = TRUE,
               raster = FALSE,
               pt.size = 0.05,
...
```{r eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE, fig.dim=c(5.5, 5.0), fig.align = 'center', results='asis'}
...
```{r eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE}
combo_ls <- list(c("nCount_RNA", "spring", "UMI per cell - SPRING Projection"),
                 c("nFeature_RNA", "spring", "Gene per cell - SPRING Projection"),
                 c("percent_mt", "spring", "Mitochondrial (%) per cell - SPRING Projection"),
                 c("nCount_RNA", "umap", "UMI per cell - UMAP Projection"),
                 c("nFeature_RNA", "umap", "Gene per cell - UMAP Projection"),
                 c("percent_mt", "umap", "Mitochondrial (%) per cell - UMAP Projection"),
                 c("nCount_RNA" ,"tsne", "UMI per cell - TSNE Projection"),
                 c("nFeature_RNA" ,"tsne", "Gene per cell - TSNE Projection"),
                 c("percent_mt", "tsne", "Mitochondrial (%) per cell - TSNE Projection"))
Q <- list()
for (i in 1:length(combo_ls)) {
    if (combo_ls[[i]][1] == "percent_mt" & max(fsobj[["percent_mt"]]) == 0) {
    message(paste("***", "maximum percent_mt = 0, no QC density plot."))
    next()
  } 
  if (combo_ls[[i]][2] %in% Reductions(fsobj)) {
    p <- plot_density(fsobj, 
                      features = combo_ls[[i]][1], 
                      reduction = combo_ls[[i]][2],
                      method = "ks", size = 1, pal = "inferno") +
      getBaseTheme() +
      theme(plot.title = element_blank(),
            axis.title = element_text(size = 12, face = "bold"),
            axis.text = element_blank(),
            ...
            legend.key.width=grid::unit(50, "points"),
...
```{r eval=eval_mrks, warning=FALSE, message=FALSE, echo=FALSE}
  adts <- rownames(fsobj@assays$ADT)
  rds <- Reductions(fsobj)[Reductions(fsobj) %nin% c("pca", "harmony")]
  adt.fsobj <- fsobj
  DefaultAssay(adt.fsobj) <- "ADT"
  Q <- list()
  for (rd in rds) {
    for (ad in adts) {
      p <- FeaturePlot(adt.fsobj, 
                       features = ad, 
                       reduction = rd, order = TRUE) +
        getBaseTheme() +
        theme(plot.title = element_blank(),
              axis.title = element_text(size = 12, face = "bold"),
              axis.text = element_blank(),
              ...
              legend.text = element_text(size = 14, face = "bold"))
      Q[[length(Q) + 1]] <- p
      names(Q)[length(Q)] <- paste(ad , "-", toupper(rd), "Projection")
    }
  } 
```

`r if (eval_mrks) { "## Visualizing Antibody-Derived Tags (ADT) Using Dimensionality Reduction {.tabset .tabset-fade .tabset-dropdown}" }`

```{r eval=eval_mrks, warning=FALSE, message=FALSE, echo=FALSE, fig.dim=c(5.5, 5.5), fig.align = 'center', results='asis'}
make_tab <- function(x) {      # function to make the tabs
  cat("\n")                    # Space
  cat("###", names(x), "\n")  # Label tab
  plot(x[[1]])                 # Display plot
...
cluster_colrs <- list()
for (i in 1:length(combo_ls)) {
  cluster_colrs[[length(cluster_colrs) + 1]] <- color_helper(set.names=sort(unique(fsobj@meta.data[[combo_ls[[i]][1]]])), 
                        panel="Set1") 
  names(cluster_colrs)[length(cluster_colrs)] <- paste(combo_ls[[i]][1], combo_ls[[i]][2], sep = "-")
}
```
...
## Visualizing Cell Populations with Dimensionality Reduction {.tabset}
...
## Cell Cluster Composition {.tabset}
...
## Statistical Dispersion Analysis of Cell Composition in Clusters across `r paste(vars, collapse=' and ')` {.tabset}
...
`r eval_mrks <- ifelse(opt$mrk_top_n != 0, TRUE, FALSE)`
...
mrks <- mrks %>%
  ...
  dplyr::top_n(opt$mrk_top_n, wt = avg_log2FC) %>%
...
mrks <- lapply(mrks, function(x){
  ...
  x <- lapply(x, function(x){
    if (length(x) == opt$mrk_top_n) return(x)
    x <- c(x, rep("", opt$mrk_top_n-length(x)))
  })
  x <- do.call(cbind, x)
})
```

`r if (eval_mrks) { "## Markers per cluster {.tabset}" }`

`r if (eval_mrks) { paste("Top", opt$mrk_top_n, "differentially expressed genes ( p_val_adj < 0.05 and pct.1 >", opt$mrk_min_pct, ") for each of the clusters") }`

`r if (eval_mrks) { "### Seurat" }`

```{r eval=eval_mrks, warning=FALSE, message=FALSE, echo=FALSE}
x <- data.frame(mrks$seurat_clusters)
names(x) <- substr(names(x), 2, nchar(names(x)))
x <- cbind(data.frame(Cluster = 1:nrow(x)), x)  
datatable(head(x, n = nrow(x)), rownames = FALSE,
          options = list(pageLength = 5, scrollX = T))
```

`r if (eval_mrks) { "### Signacx" }`
...
## Visualizing Distinct Cell Type Populations with Dimensionality Reduction {.tabset .tabset-fade .tabset-dropdown}

Note: Labels may have been removed if they overlap excessively.

```{r eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE, fig.dim=c(7.5, 5.5), fig.align = 'center', results='asis'}
make_tab <- function(x) {      # function to make the tabs
  cat("\n")                    # Space
  cat("###", names(x), "\n")  # Label tab
  plot(x[[1]])                 # Display plot
...
## Cell Type Population Composition {.tabset}
...
`r if (opt$genesets != "none") { "## Visualizing the Hierarchy of User-Provided Gene Sets" }`
...
if (opt$genesets != "none") {
  # ===================================
  layers <- list()
  gene.sets <- fsobj@misc$sargent_genesets_pos
  # ===================================
  ...
  # ===================================
  xsobj <- fsobj
  xsobj$temp <- xsobj$sargent_onto
  # print(table(xsobj$temp))
  for (x in root_cltypes) {
    xsobj$temp <- replacegrepl(xsobj$temp, x, x)  
  }
  # print(table(xsobj$temp))
  # ===================================
  gns <- gene.sets[names(gene.sets) %in% root_cltypes]
  gns <- unique(unlist(gns))
  if (length(gns) > 60) {
    gns_ls <- split(gns, ceiling(seq_along(gns)/60))
    for (i in 1:length(gns_ls)) {
      ftrs <- gns_ls[[i]]
      p <- DotPlot_helper(sobj=xsobj, ftrs=ftrs)  
      layers[[length(layers) + 1]] <- p
      names(layers)[length(layers)] <- paste("Master", i, sep = "-")
    }
  } else {
    p <- DotPlot_helper(sobj=xsobj, ftrs=gns)
    layers[[length(layers) + 1]] <- p
    names(layers)[length(layers)] <- "Master"
  }
  # ===================================
  for (cltype in sort(names(gene.sets))) {
    ...
    cells <- Cells(fsobj)[grepl(paste0("^", childs, collapse="|"), fsobj$sargent_onto)]
    # if (length(cells) == 0) { next }
    if (length(cells) <= 1) { 
      message("*** WARNING: identified an object of less than or equal to 1 cell. Skipping the DotPlot'ing...")
      next 
    }
    xsobj <- subset(fsobj, cells = cells)
    xsobj$temp <- xsobj$sargent_onto
    # print(table(xsobj$temp))
    for (x in childs) {
      xsobj$temp <- replacegrepl(xsobj$temp, x, x)  
    ...
    # print(table(xsobj$temp))
    gns <- gene.sets[names(gene.sets) %in% childs]
    gns <- unique(unlist(gns))
    if (length(gns) > 60) {
      gns_ls <- split(gns, ceiling(seq_along(gns)/60))
      for (i in 1:length(gns_ls)) {
        ...
        p <- DotPlot_helper(sobj=xsobj, ftrs=ftrs)  
        layers[[length(layers) + 1]] <- p
        names(layers)[length(layers)] <- paste(cltype, i, sep = "-")
      }
    } else {
      p <- DotPlot_helper(sobj=xsobj, ftrs=gns)
      layers[[length(layers) + 1]] <- p
      names(layers)[length(layers)] <- cltype 
    }
  }
}
```

`r if (opt$genesets != "none") { "## Evaluating Sargent Cell Type Annotation with Gene Marker Expression {.tabset .tabset-fade .tabset-dropdown}" }`

`r if (opt$genesets != "none") { "Expression level of selected genes for each group. Color indicates the average expression and size indicates the percent expressed." }`

```{r eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE, fig.dim=c(5.0, 8.5), fig.align = 'center', results='asis'}
if (opt$genesets != "none") {
  make_tab <- function(x) {      # function to make the tabs
    cat("\n")                    # Space
    cat("###", names(x), "\n")  # Label tab
...
`r eval_mrks <- !is.null(trajs)`

```{r eval=eval_mrks, warning=FALSE, message=FALSE, echo=FALSE}
trajs_rds <- list()
trajs_bx <- list()
for (i in 1:length(trajs)) {
  name_sds <- names(trajs)[i]
  rdc <- strsplit(name_sds, split="-")[[1]][2]
  grp <- strsplit(name_sds, split="-")[[1]][1]
  colrs <- cluster_colrs[[name_sds]]
  # ===================================
  p <- DimPlot(fsobj, 
               reduction = rdc,
               group.by = grp,
    ...
    guides(fill = "none", color = "none")  
  # ===================================
  curves <- slingCurves(trajs[[i]], as.df=TRUE)
  rdc <- ifelse(rdc == "umap", "umap", "tSNE")
  p <- p + geom_path(data = curves %>% arrange(Order),
                     aes(x=!!sym(paste0(rdc,"_1")) , y=!!sym(paste0(rdc,"_2")), 
                         group = Lineage), 
                     linetype = "dashed", linewidth= 0.4)
  # ===================================
  # Calculate label positions (example: using the last point of each trajectory)
  label_positions <- curves %>%
    dplyr::group_by(Lineage) %>%
    dplyr::summarize(x = last(!!sym(paste0(rdc,"_1"))),
                     y = last(!!sym(paste0(rdc,"_2"))),
                     label = paste0("Lineage-", unique(Lineage)))
  # Add labels to the plot
  set.seed(123)
  p <- p + geom_text_repel(data = label_positions, aes(x = x, y = y, label = label),
                           # nudge_x = 1, nudge_y = 1,  # Adjust nudge_x and nudge_y to position the labels appropriately
                           size = 3.5, fontface = "bold")
  # ===================================
  trajs_rds[[length(trajs_rds) + 1]] <- p
  names(trajs_rds)[length(trajs_rds)] <- paste("Seurat", toupper(rdc))
  # ===================================
  lineages <- names(slingLineages(trajs[[i]]))
  num_lineages <- length(lineages)
  # ===================================
  ftrs <- names(fsobj@meta.data)
  ftrs <- ftrs[grepl(paste0("^", name_sds, "-Lineage"), ftrs)]
  stopifnot(length(ftrs) == num_lineages)
  db_plot <- FetchData(fsobj, c("seurat_clusters", ftrs)) %>%
    tibble::rownames_to_column(var="barcode") %>%
    tidyr::gather(., key = ftrs, value = "pseudotime", -barcode, -seurat_clusters) %>%
    dplyr::group_by(ftrs) %>%
    dplyr::mutate(lineage = paste0("Lineage-", cur_group_id())) %>%
    dplyr::ungroup() %>%
    dplyr::mutate(lineage = factor(lineage, 
                                   levels=unique(lineage)[mixedorder(unique(lineage), decreasing=TRUE)], 
                                   ordered=TRUE))
  # ===================================
  p <- plot_ly(data = db_plot, x = ~pseudotime, y = ~lineage, 
        type = "box", boxpoints = FALSE, showlegend = T,
        color = ~seurat_clusters, colors = colrs) %>%
  layout(xaxis = list(title = "Pseudotime"),
         yaxis = list(title = ""),
         legend = list(title = list(text = 'Seurat Clusters')),
         boxmode = "group", boxgap = 0, boxgroupgap = 0)
  # ===================================
  trajs_bx[[length(trajs_bx) + 1]] <- p
  names(trajs_bx)[length(trajs_bx)] <- paste("Seurat", toupper(rdc))
  # ===================================
  mrks <- fsobj@misc$traj_genes
  
  mrks <- mrks %>%
    dplyr::group_by(type, Lineage) %>%
    dplyr::top_n(opt$traj_top_n, wt = logFC) %>%
    dplyr::select(type, Lineage, Gene, logFC) %>%
    dplyr::ungroup() %>%
  ...
  mrks <- lapply(mrks, function(x){
    x %>%
      ...
      dplyr::select(id, Gene, Lineage) %>%
      dplyr::mutate(Lineage = factor(Lineage, 
                                     levels=unique(Lineage)[mixedorder(unique(Lineage), decreasing=FALSE)], 
                                     ordered=TRUE)) %>%
      tidyr::spread(key=Lineage, value=Gene)
    }) 
  ...
  mrks <- lapply(mrks, function(x){
    ...
    x$id <- NULL
    x <- lapply(x, function(x){
      if (length(x) == opt$traj_top_n) return(x)
      x <- c(x, rep("", opt$traj_top_n-length(x)))
      })
    x <- do.call(cbind, x)
    })
  }
...
`r if (eval_mrks) { "# BridgeTrajectory" }`

`r if (eval_mrks) { "## Visualizing Inferred Trajectory on Manifolds {.tabset}" }`

`r if (eval_mrks) { "Note: Labels may have been removed if they overlap excessively." }`

```{r eval=eval_mrks, warning=FALSE, message=FALSE, echo=FALSE, fig.dim=c(5.5, 5.0), fig.align = 'center', results='asis'}
make_tab <- function(x) {      # function to make the tabs
  cat("\n")                    # Space
  cat("###", names(x), "\n")  # Label tab
  plot(x[[1]])                 # Display plot
...
for(i in 1:length(trajs_rds)){
  make_tab(trajs_rds[i])
}
```
...
`r if (eval_mrks) { "## Visualizing Inferred Trajectory of 'pseudotime' Across Different 'lineages' and 'clusters' {.tabset}" }`

`r if (eval_mrks & length(trajs) >= 1) { paste("###", names(trajs_rds)[1]) }`

```{r eval=eval_mrks & length(trajs) >= 1, warning=FALSE, message=FALSE, echo=FALSE, fig.dim=c(9.0, 9.0), fig.align = 'center', results='asis'}
trajs_bx[[1]]
```

`r if (eval_mrks & length(trajs) >= 2) { paste("###", names(trajs_rds)[2]) }`

```{r eval=eval_mrks & length(trajs) >= 2, warning=FALSE, message=FALSE, echo=FALSE, fig.dim=c(9.0, 9.0), fig.align = 'center', results='asis'}
trajs_bx[[2]]
```
...
`r if (eval_mrks) { "## Trajectory-based Differential Expression Analysis {.tabset}" }`

`r if (eval_mrks) { paste("Top", opt$traj_top_n, "differentially expressed genes (FDR < 0.05) along each inferred lineage.") }`

`r if (eval_mrks & length(trajs) >= 1) { paste("###", names(trajs_rds)[1]) }`

```{r eval=eval_mrks & length(trajs) >= 1, warning=FALSE, message=FALSE, echo=FALSE}
id <- which(grepl(tolower(strsplit(names(trajs_rds)[1], split=" ")[[1]][2]), names(mrks)))
x <- data.frame(mrks[[id]])
x <- cbind(data.frame(ID = 1:nrow(x)), x)
datatable(head(x, n = nrow(x)), rownames = FALSE,
          options = list(pageLength = 5, scrollX = T))
```
...
```{r eval=eval_mrks & length(trajs) >= 2, warning=FALSE, message=FALSE, echo=FALSE}
id <- which(grepl(tolower(strsplit(names(trajs_rds)[2], split=" ")[[1]][2]), names(mrks)))
x <- data.frame(mrks[[id]])
x <- cbind(data.frame(ID = 1:nrow(x)), x)
datatable(head(x, n = nrow(x)), rownames = FALSE,
          options = list(pageLength = 5, scrollX = T))
...
if (!is.null(opt$paper_url)) {
  cat(paste0("Publication: [paper](", opt$paper_url,")", collapse="\n\n"), sep="\n")
} else {
  cat(paste0("Publication: no referrence"), sep="\n")
}
```

```{r, eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE, results='asis'}
if (!is.null(opt$data_url)) {
  cat(paste0("Data Availability: [data](", opt$data_url,")", collapse="\n\n"), sep="\n")
} else {
