...
```{r table-1, eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE, fig.align = 'center'}
  ...
  #   rm.mtdata <- c(rm.mtdata, names(opt)[grepl("^scr_", names(opt))])
  # }
  data.frame(opt) %>% 
...
```{r eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE}
if (opt$tsne) {
  combo_ls <- list(c("sargent_cellstates", "umap"),
                   c("sargent_cellstates" ,"tsne"),
                   c("sargent_cellstates" ,"spring"),
                   c("signacx_cellstates", "umap"),
                   c("signacx_cellstates", "tsne"),
                   c("signacx_cellstates", "spring"))
} else {
  combo_ls <- list(c("sargent_cellstates", "umap"),
                   c("sargent_cellstates", "spring"),
                   c("signacx_cellstates", "umpa"),
                   c("signacx_cellstates", "spring"))
}
Q <- list()
for (i in 1:length(combo_ls)) {
...
plot(Q[["signacx_cellstates-spring"]])
```

#### Signacx UMPA Projection

```{r eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE, fig.dim=c(5.5, 5.5), fig.align = 'center'}
plot(Q[["signacx_cellstates-umap"]])
```

`r if (opt$tsne) { "#### Signacx TSNE Projection" }`

```{r eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE, fig.dim=c(5.5, 5.5), fig.align = 'center'}
if (opt$tsne) { plot(Q[["signacx_cellstates-tsne"]]) }
```

#### Sargent SPRING Projection

```{r eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE, fig.dim=c(5.5, 5.5), fig.align = 'center'}
plot(Q[["sargent_cellstates-spring"]])
```

#### Sargent UMAP Projection

```{r eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE, fig.dim=c(5.5, 5.5), fig.align = 'center'}
plot(Q[["sargent_cellstates-umap"]])
```

`r if (opt$tsne) { "#### Sargent TSNE Projection" }`

```{r eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE, fig.dim=c(5.5, 5.5), fig.align = 'center'}
if (opt$tsne) { plot(Q[["sargent_cellstates-tsne"]]) }
```
...
```{r, eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE}
annots <- c("signacx_cellstates", "sargent_cellstates")
Q <- list()
for (ant in annots) {
    df <- data.frame(table(fsobj[[ant]]))
...
```{r eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE, fig.dim=c(17.5, 9.5), fig.align = 'center'}
plot(Q[[1]])
```
...
### Sargent gene-sets hierarchy

```{r eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE, max.height='300px', results='asis'}
gset <- fsobj@misc$sargent_genesets_pos
n <- max(unlist(lapply(strsplit(names(gset), split = "\\."), length)))
df <- suppressWarnings(data.frame(parent=sub('[.][^.]+$', '', names(gset)), 
                                  name=names(gset), 
                                  stringsAsFactors=FALSE) %>%
                         dplyr::bind_rows(data.frame(parent = "TBD",
                                                     name = "TBD")) %>%
                         tidyr::separate(., col = name, into = LETTERS[1:n], sep = "\\."))
df$pathString <- apply(df[,LETTERS[1:n]], 1, function(x){
  x <- x[!is.na(x)]
  x <- paste(x, collapse = "/")  
  paste("sargent", x, sep = "/")  
})
