...
```{r eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE, fig.width=17.5, fig.height=4.5, fig.align = 'center'}
  ...
  for (tp in unique(db_plot$type)) {
    p <- ggplot(data = filter(db_plot, type == tp)) +
      ...
      xlab("") + ylab("Density") +
      geom_density(aes(x = feature, group = sample_id, color = sample_id), 
...
```{r eval=eval_mrks, warning=FALSE, message=FALSE, echo=FALSE}
  ...
  rds <- Reductions(fsobj)[Reductions(fsobj) %nin% c("pca")]
  adt.fsobj <- fsobj
...
`r if (opt$mrk_top_n != 0) { "## Markers per cluster {.tabset}" }`

`r if (opt$mrk_top_n != 0) { paste("Top", opt$mrk_top_n, "differentially expressed genes ( p_val_adj < 0.05 and pct.1 >", opt$mrk_min_pct, ") for each of the clusters") }`

`r if (opt$mrk_top_n != 0) { "### Seurat" }`
...
for (i in 1:length(trajs)) {
  ...
  Q[[length(Q) + 1]] <- p
  names(Q)[length(Q)] <- paste("Seurat", toupper(rdc), "Projection")
}
```

`r if (eval_mrks) { "# BridgeTrajectory" }`

`r if (eval_mrks) { "## Visualizing Infered Trajectory {.tabset}" }`
