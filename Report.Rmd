...
for (i in 1:length(trajs)) {
  ...
  # rdc <- strsplit(names(trajs)[i], split="-")[[1]][2]
  # grp <- strsplit(names(trajs)[i], split="-")[[1]][1]
  # colrs <- color_helper(set.names=sort(unique(fsobj@meta.data[[grp]])), 
  #                       panel="Set1")
  # ===================================
  p <- DimPlot(fsobj, 
               reduction = rdc,
  ...
  db_plot <- FetchData(fsobj, c("seurat_clusters", ftrs)) %>%
    ...
    dplyr::ungroup() %>%
    dplyr::mutate(lineage = factor(lineage, levels=lineage[mixedorder(lineage)], ordered=TRUE))
  # ===================================
